...
Vstupom siete je vektor \begin{math}x\end{math}, ktorý je spojený s neurónmi v prvej vrstve. Spojenia medzi neurónmi sa nazývajú váhy. Na základe neurónov a váh sa počítajú hodnoty neurónov v nasledujúcej vrstve. Matematické vyjadrenie na získanie hodnôt neurónov v nasledujúcej vrstve je opísané formulou:
\begingroup
...
Bežné neurónové siete nepracujú dobre s väčšími údajmi. Napríklad pre obraz veľkosti 32x32 pixlov s troma farebnými kanálmi má jeden plne pripojený neurón v prvej skrytej vrstve 3072 váh (32x32x3). Pre väčší veľkosti, napríklad 420x420x3 by mal jeden neurón 529 200 spojení, ktorých trénovanie vyžaduje veľa výpočtového výkonu.

Cieľom CNN (konvolučných neurónových sietí) je zmenšiť obraz tak, aby sa dal ľahšie spracovať bez straty príznakov, ktoré sú dôležité na čo najpresnejší výsledok. V porovnaní s bežnými neurónovými sieťami majú konvolučné siete zmenenú štruktúru vrstiev. Zahŕňajú tri typy vrstiev: konvolučnú vrstvu, pooling vrstvu a plne prepojenú vrstvu.
...
Po tom čo údaje prejdu cez všetky konvolučné a združovacie vrstvy, vstupujú do plne prepojenej vrstvy. Tá je v zásade totožná s neurónovou sieťou, ktorú sme uvádzali v úvode kapitoly. Plne prepojená vrstva dostáva na vstup jednorozmerné údaje, z ktorých je vypočítaný konečný výsledok siete.
...
V súčasnosti sa na detekciu používajú prerobené klasifikátory. Systémy na detekciu objektu využijú klasifikátor objektu a vyhodnocujú ho na rôznych miestach a mierkach v testovanom obraze. Niektoré systémy ako napr. DPM \textit{(deformable parts model)} využívajú metódu pohyblivých okien, ktoré klasifikátor kontroluje na rovnomerne veľkých plochách celého obrazu.
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.6\textwidth]{images/02/yolo.png}
    \caption{YOLO}
    \label{img:ch2}
\end{figure}
...
%V súčasnosti sa na detekciu využívajú klasifikátory objektov, ktoré vyhodnocujú objekt na rôznych miestach a mierkach v testovanom obraze. Niektoré systémy, ako napríklad DPM (deformable parts model), využívajú metódu pohyblivých okien, ktoré klasifikátor kontroluje na rovnomerne veľkých plochách celého obrazu. Modernejším prístupom by bolo použitie CNN na vygenerovanie potenciálnych hraničných oblastí a použitie klasifikátora len na tieto oblasti.
%
%Tieto spomenuté postupy sú pomalé a je ťažké ich optimalizovať, pretože každá individuálna súčasť musí byť trénovaná samostatne. YOLO sa stavia ku hľadaniu objektov komplexne a rieši detekciu objektov od vstupu až po výsledné hraničné oblasti. Jedna konvolučná sieť predikuje hraničné oblasti a pravdepodobnosti príslušnosti k jednotlivým triedam. Tento proces prebieha naraz nad celým obrazom a je analyzovaný napriek všetkými triedami. Aj napriek vysokej výkonnosti v porovnaní s ostatným sieťami ako napríklad Fast R-CNN, nie je YOLO dokonalá sieť a problémy má hlavne s hľadaním objektov menších rozmerov alebo takých objektov, ktoré sa v obraze vyskytujú v skupinkách \cite{RedmonJoseph2016YOLO}.
%
%\begin{figure}[ht]
%    \centering
%    \includegraphics[width=0.7\textwidth]{images/02/yolo.png}
%    \caption{Základný princíp fungovania siete YOLO \cite{RedmonJoseph2016YOLO}.}
%    \label{img:yolo}
%\end{figure}
%
%Vstupný obraz je rozdelený na mriežku s rozmermi S x S. Na to aby mohlo byť určené, že objekt patrí ku konkrétnemu štvorcu v mriežke je vyrátavané skóre, ktoré určuje ako si je model istý, že našiel v danej oblasti hľadný objekt. Ak nastane situácia, že bol objekt objavený a spadá do konkrétneho štvorca z mriežky, tak je od toho momentu daný štvorec zodpovedný za detegovanie daného objektu.
%
%Je bežným javom, že sa pre jeden detegovaný objekt vygenerujú viaceré hraničné oblasti. Tie bývajú umiestnené na rôznych pozíciách často sa prekrývajú. Na tento účel slúži nemaximálna supresia, ktorej výstupom sú  a jej výstupom je len jedna hraničná oblasť na každý detegovaný objekt.


\section{Výzvy neurónových sietí}
Použitie neurónových sieti mnohokrát prináša veľmi dobré výsledky pre mnohé projekty a úroveň \textit{state-of-art} sa rokmi zlepšuje. Bohužiaľ mnohé práce narážajú na niekoľko problémov, ktoré bránia dosiahnuť dobrú presnosť neurónovej siete \cite{RosinPaulL2019RIAa}.
...
Vo všeobecnosti si trénovanie môže dovoliť trvať dlhý čas, no výpočtový čas je často veľmi dôležitým kritériom výkonu, ktorý môže byť hlavnou požiadavkou pre niektoré aplikácie. Ako príklad sa dajú uviesť časovo kritické aplikácie, akými sú autonómne riadenie a sledovacie systémy, ktoré musia nevyhnutne fungovať v reálnom čase. Iné aplikácie, napríklad náš projekt, si môže dovoliť pomalšie časy výpočtu. Často tak existuje kompromis medzi výpočtovým časom a výkonom siete. Dlhý čas trénovania môže byť v niektorých prípadoch predsa len problém. Nie každý si môže dovoliť dlho čakať na výsledky, prípadne nemá zdroje, ktoré by zvládli natrénovať model v prijateľnom čase alebo s dostatočne veľkými parametrami.
