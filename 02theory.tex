...
% chcelo by to možno dáky vzorec
...
% todo definovať niektoré parametre a naznačiť ako sa tým mení opísaný princíp; ideálne tie, ktoré potom budeme ladiť v riešení
...
% todo, pridať auto labeling, SAM a Grounding Dino 
...
% todo, pridať nejakú bližšiu analýzu
...
V neposlednom rade výsledky komplikujú podmienky, pri ktorých je hľadaný objekt zachytený. Príkladom môže byť autonómne riadenie vozidla, ktoré musí fungovať počas celého dňa, čiže za každých svetelných podmienok a to aj v extrémnych situáciách. Oklúzia nastáva keď objekt nie je viditeľný celý kvôli jeho prekrytiu. Deformácia nastáva hlavne pre objekty, ktoré nie sú pevné a tak sa môžu vyskytovať v rôznych formách. TODO zlepšiť odsek, obrázok netreba.

% The occlusion problem occurs when an object in an image is partially or completely obscured by another object, making it difficult for the neural network to correctly identify or classify the object. The illumination problem arises when the lighting conditions in an image are poor or inconsistent, making it difficult for the neural network to distinguish between different objects or features. Finally, the deformation problem occurs when an object in an image is not in its typical or expected shape or position, making it difficult for the neural network to correctly identify or classify it.
